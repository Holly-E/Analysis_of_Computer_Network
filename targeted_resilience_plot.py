...
Code for loading the targeted resilience comparison graph.
"""
...
ax1.plot(x, network_y, 'orange', label='Computer network sim')
ax1.plot(x, er_y, 'blue', label='ER graph, p = 0.00397')
ax1.plot(x, upa_y, 'purple', label='UPA graph, m = 3')

ax1.set_title('Resilience Comparison - Targeted Attack', weight=600)
ax1.set_xlabel('Number of Nodes Removed')
ax1.set_ylabel('Size of Largest Connect Component')
plt.legend(loc='best')

plt.savefig('Images/Resilience Comparison Targeted V2', orientation='landscape')
plt.show()
